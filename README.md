...
- The portfolio case studies include verified software architecture, real-time control, planning, learning-based control, and hardware evidence. Imitation learning and Diffusion Policy are presented only as future technology-transfer opportunities.
- The public headline, professional summary, current-role evidence, and experience dates are aligned with the current LinkedIn profile while private address and phone details remain excluded.
- Eight interactive simulations cover a two-link arm, Copter 1-3, Drone 4/6/8, and the 18-rotor TaxiDrone in a browser worker with editable controller gains, CAD, links, coordinate frames, center-of-mass markers, multiple views, response plots, and a red XYZ target marker. Parameters are arranged by control loop in three-column groups. Every field has an information icon, and every PID exposes signed initial integral and antiwindup settings as percentages of the fixed actuator stall input. The initial value is clamped by the antiwindup limit. Only one calculation can run at a time. All seven rotor systems use nonlinear articulated-body dynamics, physical rotor thrust and drag, motor delay and limits, encoder quantization, state estimation, and direct unnormalized rotor-group mixing.

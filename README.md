...
- Eight Dynamics Forge Web Demos cover a two-link arm, Copter 1-3, Drone 4/6/8, and the 18-rotor TaxiDrone in a browser worker with editable controller gains, CAD, links, coordinate frames, center-of-mass markers, multiple views, and response plots. Drone 6, Drone 8, and TaxiDrone include a second time-history plot combining X and Y responses with their targets; their independent roll and pitch commands are fixed at zero while the position loop generates the attitude needed for lateral motion. Their compact control panels place gains first, expose a shared integrator anti-windup maximum, and group all targets last. Every run lasts 10 seconds. All seven rotor systems use Featherstone articulated-body dynamics, physical rotor thrust and drag, motor delay and limits, encoder quantization, state estimation, and direct unnormalized rotor-group mixing.
